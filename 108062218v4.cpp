...
bool position_X_check(Point p){
    if(p == Point(0,1) || p == Point(1,0) || p == Point(1,1) ||
       p == Point(0,6) || p == Point(1,7) || p == Point(1,6) ||
       p == Point(6,0) || p == Point(7,1) || p == Point(6,1) ||
       p == Point(6,7) || p == Point(7,6) || p == Point(6,6))
    return true;
    else return false;
}
void write_valid_spot(ofstream& fout) {
        ...
        for(auto np : cur_board.next_valid_spots){
            ...
            if(next_a > min_num){
                if(check_opponent_corner(next,player) || position_X_check(np))
                    bad_idea = np;
                else{
                min_num = next_a;
                p = np;
